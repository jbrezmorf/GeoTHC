...
Hranice $\partial\Omega_d$ je rozdělena na části $\Gamma^{I,c}_d\cup\Gamma^{D,c}_d\cup\Gamma^{N,c}_d\cup\Gamma_d^{R,c}$, které mohou být časově závislé.
První segment $\Gamma^{I,c}_d$ je dále rozdělen na část vstupní a výstupní:
\begin{align*}
\Gamma^{I,c+}_d(t) &= \{\vc x\in \partial\Omega_d\where \vc q(t,\vc x)\cdot\vc n(\vc x)<0\},\\
\Gamma^{I,c-}_d(t) &= \{\vc x\in \partial\Omega_d\where \vc q(t,\vc x)\cdot\vc n(\vc x)\ge 0\},
\end{align*}
kde $\vc n$ značí vektor vnější jednotkové normály k $\partial\Omega_d$.
Jsou předepsány následující okrajové podmínky:
\begin{align*}
c^i &= c^i_D &&\mbox{ na }\Gamma^{I,c+}_d\cup\Gamma_d^{D,c}, & \mbox{(Dirichlet)}\\
-\th\delta\D^i\nabla c^i\cdot\vc n &= 0 &&\mbox{ na }\Gamma_d^{I,c-}, & \mbox{(homogenní Neumann)}\\
-\th\delta\D^i\nabla c^i\cdot\vc n &= f^i_N &&\mbox{ na }\Gamma_d^{N,c}, & \mbox{(nehomogenní Neumann)}\\
-\th\delta\D^i\nabla c^i\cdot\vc n &= \sigma^i_R(c^i-c^i_D) &&\mbox{ na }\Gamma_d^{R,c}. &\mbox{(Robin/Newton)}
\end{align*}
